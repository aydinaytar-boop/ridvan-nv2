...
export interface TodayTimes {
  sabah: string; gunes: string; ogle: string; ikindi: string; aksam: string; yatsi: string; sabahKamet: string;
}
...
export const SETTINGS = {
  sabahKamet: "06:15",
  kametSureleri: {
    ogle: 8, ikindi: 8, aksam: 3, yatsi: 8,
  ...
  bayramlar: [
    { ad_tr: "Ramazan Bayramı", ad_de: "Eid al-Fitr", tarih: "2026-03-20", tarih2: "2026-03-22" },
    { ad_tr: "Kurban Bayramı", ad_de: "Eid al-Adha", tarih: "2026-05-27", tarih2: "2026-05-30" },
  ],
};

let _sabahKametSaati = SETTINGS.sabahKamet;
export function getSabahKametSaati(): string { return _sabahKametSaati; }
export function setSabahKametSaati(val: string) { _sabahKametSaati = val; }
...
export function computeFlow(now: Date, times: TodayTimes): FlowState {
  ...
  for (const vakit of ezanVakitler) {
    ...
    if (secSinceEzan >= 0 && secSinceEzan < 60) {
      return { phase: "ezan", currentVakit: vakit, nextVakit: next, nextVakitTime: nextTime, countdownSeconds: 0, ezanElapsed: secSinceEzan, blackoutRemaining: 0, activeEzanVakit: vakit, kametCountdown: 0, kametAlertActive: false };
    }
    if (now >= ezanDate && secSinceEzan >= 60 && now < kametDate) {
      return { phase: "kamet_countdown", currentVakit: vakit, nextVakit: next, nextVakitTime: nextTime, countdownSeconds: 0, ezanElapsed: secSinceEzan, blackoutRemaining: 0, activeEzanVakit: vakit, kametCountdown: diffSeconds(now, kametDate), kametAlertActive: false };
    }
    if (now >= kametDate && now < kametAlertEnd) {
      return { phase: "kamet_alert", currentVakit: vakit, nextVakit: next, nextVakitTime: nextTime, countdownSeconds: 0, ezanElapsed: secSinceEzan, blackoutRemaining: 0, activeEzanVakit: vakit, kametCountdown: 0, kametAlertActive: true };
    }
    if (now >= kametAlertEnd && now < blackoutEnd) {
      return { phase: "blackout", currentVakit: vakit, nextVakit: next, nextVakitTime: nextTime, countdownSeconds: 0, ezanElapsed: secSinceEzan, blackoutRemaining: diffSeconds(now, blackoutEnd), activeEzanVakit: vakit, kametCountdown: 0, kametAlertActive: false };
    }
  }

  const countdown = next ? diffSeconds(now, timeToDate(nextTime, now)) : 0;
  return { phase: "normal", currentVakit: current, nextVakit: next, nextVakitTime: nextTime, countdownSeconds: Math.max(0, countdown), ezanElapsed: 0, blackoutRemaining: 0, activeEzanVakit: null, kametCountdown: 0, kametAlertActive: false };
}