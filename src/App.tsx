...
function miladiTRStr(d: Date) {
  return `${GUNLER_TR[d.getDay()]}, ${d.getDate()} ${AYLAR_TR[d.getMonth()]} ${d.getFullYear()}`;
}

function miladiDEStr(d: Date) {
  return `${GUNLER_DE[d.getDay()]}, ${d.getDate()}. ${AYLAR_DE[d.getMonth()]} ${d.getFullYear()}`;
}
...
export default function App() {
  ...
  useEffect(() => {
    if (configLoaded) localStorage.setItem("hicriOffset", String(hicriOffset));
  }, [hicriOffset, configLoaded]);

  useEffect(() => {
    if (configLoaded) localStorage.setItem("bayramInputs", JSON.stringify(bayramInputs));
          ...
          {showDuyuruForm && (
            <>
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
                <span style={{ color: "#f5d78e", fontSize: 16 }}>{lang === "tr" ? "Türkçe Duyuru" : "Türkische Ankündigung"}</span>
                <textarea value={duyuruTR} onChange={(e) => setDuyuruTR(e.target.value)} style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "2px solid #c9a66b", background: "#0a3d2e", color: "#f5d78e", fontSize: 16, minHeight: 80, resize: "vertical" }} />
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <span style={{ color: "#f5d78e", fontSize: 16 }}>{lang === "tr" ? "Almanca Duyuru" : "Deutsche Ankündigung"}</span>
                <textarea value={duyuruDE} onChange={(e) => setDuyuruDE(e.target.value)} style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "2px solid #c9a66b", background: "#0a3d2e", color: "#f5d78e", fontSize: 16, minHeight: 80, resize: "vertical" }} />
              </div>
            </>
          )}
        </div>
        <div style={{ color: "#6a9e78", fontSize: 14, textAlign: "center", maxWidth: 600, marginTop: 8 }}>
          ℹ️ {lang === "tr" ? "TV'den yapılan değişiklikler bu cihazda saklanır. Tüm TV'ler için GitHub'da public/config.json dosyasını güncelleyin." : "Änderungen von TV werden auf diesem Gerät gespeichert. Für alle TVs aktualisieren Sie public/config.json auf GitHub."}
        </div>
    ...
    <div className="tv-safe-area">
      <div className="outer-frame" style={{ display: "flex", flexDirection: "column" }}>
        ...
        <div className="main-panels" style={{ display: "flex", flex: 1 }}>
          ...
          <div className="panel" style={{ flex: 1, display: "flex", flexDirection: "column", background: "#0a3d2e" }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
              {vakitList.map(({ key, ezan, kamet }) => {
                ...
                return (
                  <div key={key} className={isActive ? "active-vakit-row" : ""} style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 1fr", padding: "0 20px", flex: 1, borderBottom: "1px solid #c9a66b33", background: isActive ? undefined : isNext ? "rgba(201,166,107,0.07)" : "transparent", borderLeft: isActive ? "7px solid #c9a66b" : "7px solid transparent", alignItems: "center", transition: "background 0.3s" }}>
              ...
              {weekendMsg && (
                <div style={{ padding: "8px 16px", color: "#c9a66b", fontSize: 16, fontStyle: "italic", flexShrink: 0, borderTop: "1px solid #c9a66b33", lineHeight: 1 }}>{lang === "tr" ? "Haftasonu eğitimi sebebiyle öğle namazı 13:00 olarak ayarlanmıştır." : "Das Mittagsgebet ist aufgrund des Wochenendunterrichts auf 13:00 Uhr festgelegt."}</div>
              )}
            </div>
          </div>

          {/* ORTA PANEL */}
          <div className="panel" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#0a3d2e", gap: 28 }}>
            {isKametAlert ? (
              ...
              <>
                <div style={{ background: "#c9a66b", textAlign: "center", padding: "12px 48px", color: "#0a3d2e", fontSize: 35, fontWeight: 900, letterSpacing: 3, borderRadius: 8, lineHeight: 1 }}>{lang === "tr" ? "GÜNÜN VAKTİ" : "AKTUELLE GEBETSZEIT"}</div>
                <div style={{ color: "#f5d78e", fontSize: isEzan ? 108 : 98, fontWeight: 900, letterSpacing: 3, lineHeight: 1, animation: isEzan ? "pulse 1s infinite" : "none" }}>{currentLabel}</div>
                {!isKametCountdown && (
                  <>
                    <div style={{ background: "#c9a66b", textAlign: "center", padding: "12px 48px", color: "#0a3d2e", fontSize: 30, fontWeight: 900, letterSpacing: 3, borderRadius: 8, lineHeight: 1, marginTop: 8 }}>{lang === "tr" ? "SONRAKI VAKİT" : "NÄCHSTE GEBETSZEIT"}</div>
                    <div style={{ color: "#f5d78e", fontSize: 65, fontWeight: 700, lineHeight: 1 }}>{nextLabel} — {nextTime}</div>
                  </>
                )}
                {isKametCountdown && (
                  <div style={{ textAlign: "center", marginTop: 8 }}>
                    <div style={{ background: "#c9a66b", textAlign: "center", padding: "12px 48px", color: "#0a3d2e", fontSize: 30, fontWeight: 900, letterSpacing: 3, borderRadius: 8, lineHeight: 1 }}>{lang === "tr" ? "KAMETE KALAN SÜRE" : "ZEIT BIS ZUM IQÂMAT"}</div>
                    <div style={{ color: "#f5d78e", fontSize: 60, fontWeight: 700, marginTop: 16, lineHeight: 1 }}>{kametVakit ? VAKIT_NAMES[lang][kametVakit] : ""}</div>
                  </div>
                )}
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 24 }}>
                  {[
                    ...
                    item === null ? (
                      <span key={i} style={{ color: "#f5d78e", fontSize: 61, fontWeight: 900, lineHeight: 1, animation: "pulse 1s infinite" }}>:</span>
                    ) : (
                      <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", background: "#0e5c3a", border: "4px solid #c9a66b", borderRadius: 16, padding: "16px 32px", minWidth: 125 }}>
                        <span style={{ color: "#f5d78e", fontSize: 77, fontWeight: 900, fontFamily: "monospace", lineHeight: 1 }}>{item.val}</span>
                        <span style={{ color: "#c9a66b", fontSize: 18, marginTop: 8, letterSpacing: 1, lineHeight: 1 }}>{item.label}</span>
                      </div>
                    )
                  )}
                </div>
                {flow.currentVakit === "sabah" && !isEzan && !isKametCountdown && (
                  <div style={{ textAlign: "center", marginTop: 24 }}>
                    <div style={{ color: "#c9a66b", fontSize: 30, letterSpacing: 3, lineHeight: 1 }}>{lang === "tr" ? "GÜNEŞE KALAN" : "BIS SCHURUQ"}</div>
                    <div style={{ color: "#a8c8b0", fontSize: 50, fontWeight: 700, lineHeight: 1 }}>{fmt2(gunesKalanH)}:{fmt2(gunesKalanM)}:{fmt2(gunesKalanS)}</div>
                  </div>
                )}
              </>
            )}
            {bayram.visible && (
              <div style={{ marginTop: 28, padding: "16px 48px", background: "#c9a66b22", border: "3px solid #c9a66b", borderRadius: 12, color: "#f5d78e", fontSize: 35, fontWeight: 700, textAlign: "center", lineHeight: 1 }}>🎉 {lang === "tr" ? bayram.bayram?.ad_tr : bayram.bayram?.ad_de}</div>
            )}
          </div>
          ...
          <div className="panel" style={{ flex: 1, display: "flex", flexDirection: "column", background: "#0a3d2e" }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", borderBottom: "3px solid #c9a66b" }}>
              <div style={{ background: "#c9a66b", textAlign: "center", padding: "10px 0", color: "#0a3d2e", fontSize: 20, fontWeight: 900, letterSpacing: 3 }}>
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px 16px", gap: 16 }}>
                {isEzan ? (
                  <>
                    <div style={{ color: "#f5d78e", fontSize: 36, textAlign: "right", lineHeight: 1.8, direction: "rtl", fontFamily: "serif", width: "100%" }}>
                      {EZAN_DUASI.arabic}
                    </div>
                    <div style={{ color: "#a8c8b0", fontSize: 18, textAlign: "center", lineHeight: 1.6 }}>
                  ...
                  <>
                    {dailyDua.source && (
                      <div style={{ background: "transparent", border: "2px solid #c9a66b", borderRadius: 8, padding: "4px 16px", color: "#c9a66b", fontSize: 14, fontWeight: 700, letterSpacing: 3 }}>
                        {dailyDua.source}
                      </div>
                    )}
                    <div style={{ color: "#f5d78e", fontSize: 36, textAlign: "right", lineHeight: 1.8, direction: "rtl", fontFamily: "serif", width: "100%" }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px 16px" }}>
                <div style={{ color: "#f5d78e", fontSize: 20, textAlign: "center", lineHeight: 1.6, width: "100%", whiteSpace: "pre-wrap" }}>
        ...
        <div className="bottom-bar" style={{ background: "linear-gradient(180deg,#0a3d2e 0%,#072d20 100%)", borderTop: "4px solid #c9a66b", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 32px", cursor: "default", position: "relative" }}>
          ...
          <div style={{ border: "3px solid #c9a66b", borderRadius: 8, padding: "6px 12px", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#ffffff", boxShadow: "0 0 0 1px #c9a66b" }}><img src="/img/logo.png" alt="Ridvan Camii Logo" style={{ height: 52, objectFit: "contain" }} /></div>
        </div>
      </div>
    </div>
  );
