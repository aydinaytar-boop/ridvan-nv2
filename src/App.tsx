...
/* -------------------------------------------------------
   APP BAŞLANGICI
   ------------------------------------------------------- */
>>>>>>> 7e0e7736c7e4c83f0cfa5cb4a5091ad3a3cec2b2
...
<<<<<<< HEAD
  ...
  useEffect(() => {
    ...
    if (savedBayram) {
      ...
      } catch (e) { console.error("Bayram yüklenemedi", e); }
    }

    setConfigLoaded(true);
  }, []);

  // --- 2. DEĞİŞİKLİKLERİ OTOMATİK KAYDET (AUTO SAVE) ---
  // Herhangi bir ayar değiştiğinde localStorage'a yazar
  useEffect(() => {
    if (configLoaded) {
...
<<<<<<< HEAD
  // --- KAYDETME FONKSİYONU (MANUAL SAVE BUTTONU İÇİN) ---
  // Not: Yukarıdaki useEffect otomatik kaydediyor, 
  // bu fonksiyon butona basılınca ek işlem yapmak (örn: uyarı vermek) için.
  const handleSaveAndClose = () => {
    // Veriler zaten yukarıdaki useEffect ile anlık kaydediliyor.
    // Burada sadece ayarları kapatıyoruz.
    setShowSettings(false);
    
    // İstersen buraya bir "toast" veya console log ekleyebilirsin.
    console.log("Ayarlar kapatıldı ve veriler hafızada.");
  };
