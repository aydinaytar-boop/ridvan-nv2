...
if (data.dynamic?.prayerTimes) {
  setApiPrayerTimes(data.dynamic.prayerTimes);
      ...
      setSabahKametMins(timeToMinutes(sk));
  ...
  useEffect(() => {
    ...
    localStorage.setItem("manuelSabahKamet", sk);
    setSabahKametSaati(sk);
  }, [sabahKametMins, configLoaded]);
  ...
  useEffect(() => {
    setSabahKametSaati(minutesToTime(sabahKametMins));
  }, [sabahKametMins]);
  ...
  const vakitList: { key: VakitKey; ezan: string; kamet: string | null }[] = [
    { key: "sabah", ezan: times.sabah, kamet: minutesToTime(sabahKametMins) },
