  ...
  if (isBlackout) {
    return (
      <div style={{ width: "100vw", height: "100vh", background: "#000", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 32 }}>
        <img src="/img/telefonukapat.png" alt="Lütfen telefonunuzu kapatın" style={{ maxWidth: "80%", maxHeight: "75vh", objectFit: "contain" }} onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />
    ...
    <div className="tv-safe-area">
      <div className="outer-frame" style={{ display: "flex", flexDirection: "column" }}>
        ...
        <div className="main-panels" style={{ display: "flex", flex: 1 }}>
          ...
          {/* SAĞ PANEL */}
          <div className="panel" style={{ flex: 1, display: "flex", flexDirection: "column", background: "#0a3d2e" }}>
            <div style={{ background: "#c9a66b", textAlign: "center", padding: "10px 0", color: "#0a3d2e", fontSize: 20, fontWeight: 900, letterSpacing: 3 }}>{isEzan ? (duaLang === "tr" ? "EZAN DUASI" : "ADHAN-GEBET") : (lang === "tr" ? "GÜNÜN DUASI" : "DUA DES TAGES")}</div>
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 20px", gap: 20 }}>
              {isEzan ? (
                <>
                  <div style={{ color: "#f5d78e", fontSize: 28, textAlign: "right", lineHeight: 1.8, direction: "rtl", fontFamily: "serif" }}>{EZAN_DUASI.arabic}</div>
                  <div style={{ color: "#a8c8b0", fontSize: 16, textAlign: "center", lineHeight: 1.6 }}>{duaLang === "tr" ? EZAN_DUASI.tr : EZAN_DUASI.de}</div>
                </>
              ) : (
                <>
                  {dailyDua.source && (
                    <div style={{ background: "transparent", border: "2px solid #c9a66b", borderRadius: 8, padding: "4px 16px", color: "#c9a66b", fontSize: 14, fontWeight: 700, letterSpacing: 3 }}>{dailyDua.source}</div>
                  )}
                  <div style={{ color: "#f5d78e", fontSize: 30, textAlign: "right", lineHeight: 1.8, direction: "rtl", fontFamily: "serif", width: "100%" }}>{dailyDua.ar}</div>
                  <div style={{ color: "#a8c8b0", fontSize: 16, textAlign: "center", lineHeight: 1.6 }}>{lang === "tr" ? dailyDua.tr : dailyDua.de}</div>
                </>
              )}
            </div>
          </div>
